...
const formatNotesForInput = (notes = []) =>
  notes
    .filter(Boolean)
    .map((note) => note.trim())
    .filter(Boolean)
...
const normalizeDuration = (raw = "") => raw.trim().replace(/\s*[-–]\s*/g, "-");
...
const AdminPlans = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "duration") {
      setForm((f) => ({ ...f, duration: normalizeDuration(value) }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = {
        ...
        features: parseFeaturesFromInput(form.features),
        notes: parseNotesFromInput(form.notes),
      };
      if (editingId) {
        await axios.put(`/api/auth/plans/${editingId}`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setMessage("Plan updated!");
      } else {
        await axios.post("/api/auth/plans", data, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleEdit = (plan) => {
    setForm({
      ...
      features: formatFeaturesForInput(plan.features),
      notes: formatNotesForInput(plan.notes),
    });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`/api/auth/plans/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (plan) =>
          ...
          plan.notes?.some((n) => n.toLowerCase().includes(term)),
      );
    }
  ...
  return (
    <div className="text-gray-200 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h2 className="text-success-500 font-black text-2xl sm:text-3xl mb-4 md:mb-0">
          Manage Plans
        </h2>
        <div className="text-sm text-gray-400">
          ...
          of <span className="text-gray-300 font-bold">{plans.length}</span>{" "}
          plans
        </div>
      </div>
      ...
      <AdminFilterBar
        ...
        searchPlaceholder="Search plans by name, features, or notes..."
        filters={[
          {
            key: "highlight",
            label: "Status",
      ...
      <form
        ...
        className="bg-gray-800 rounded-xl p-4 sm:p-6 lg:p-8 mb-6 shadow-lg"
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <input
            ...
            value={form.duration || ""}
            onChange={handleChange}
            placeholder="Duration (e.g. 30 or 30-40 days)"
            title="Use a number (30) or range (30-40)"
            pattern="^\s*\d+\s*([-–]\s*\d+)?\s*$"
            required
            className="px-4 py-2 rounded-lg bg-gray-700 text-gray-200 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
          />
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          {message && (
            <span
              className={`text-sm sm:text-base ${message.includes("Error") ? "text-red-400" : "text-green-400"}`}
            >
              {message}
            </span>
          )}
      ...
      <h3 className="text-success-500 font-semibold text-xl sm:text-2xl mb-4">
        Existing Plans
      </h3>
