...
const planBenefits = [
  {
    ...
    description:
      "High-quality websites built with modern technologies and best practices",
    icon: "star",
  },
  {
    title: "Fast Delivery",
...
function Plans() {
  ...
  const openNativeShare = async (plan) => {
    try {
      ...
      if (e.name !== "AbortError")
        setShareNotice("Could not open share sheet.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-text-main pt-20">
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {planBenefits.map((benefit, index) => (
              <div
                key={benefit.title}
                className={`bg-white rounded-xl p-6 border border-gray-200 shadow-sm hover:shadow-md hover:border-primary-200 transition-all duration-300 hover:-translate-y-1 text-center ${
                  isVisible
      ...
      <section
        id="plans"
        className="py-12 md:py-20 bg-gray-50 relative overflow-hidden scroll-mt-24"
      >
        {/* Background Pattern */}
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 p-6 sm:p-8 max-w-md mx-auto">
              ...
              <p className="text-text-muted mb-6">
                Currently no pricing plans are available. Please check back
                later or contact us for custom pricing.
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
              {sortedPlans.map((plan, index) =>
                (() => {
                  const includedItems = toLineItems(plan.features);
                  const notIncludedItems = toLineItems(plan.notes);
                  return (
                    <div
                      key={plan._id || index}
                      id={`plans-${slugify(plan.name)}`}
                      className={`group relative bg-white rounded-2xl p-6 md:p-8 border-2 transition-all duration-300 cursor-pointer hover:-translate-y-2 hover:shadow-lg ${
                        plan.highlight
                          ? "border-accent-400 shadow-md bg-amber-50/60 ring-1 ring-accent-200/60"
                          : "border-gray-200 hover:border-primary-300"
                      } ${selectedPlan?._id === plan._id ? "border-primary-600 shadow-md" : ""} ${
                        isVisible
                          ? "opacity-100 translate-y-0"
                          : "opacity-0 translate-y-8"
                      }`}
                    ...
                    >
                      ...
                      {plan.oldPrice && (
                        <div className="absolute top-2 left-2 bg-danger-500 text-white px-3 py-1.5 rounded-full text-xs font-bold shadow-sm z-10">
                          {Math.round(
                            ((plan.oldPrice - plan.price) / plan.oldPrice) *
                              100,
                          )}
                          % OFF
                        </div>
                      ...
                      {notIncludedItems.length > 0 && (
                        <div className="mb-6">
                          <h4 className="flex items-center gap-2 text-lg font-semibold text-amber-700 mb-4">
                            <Icon
                              name="warning"
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md max-h-[100dvh] sm:max-h-[min(90dvh,calc(100dvh-2rem))] flex flex-col border border-gray-200 overflow-hidden">
            <div className="flex items-start justify-between gap-3 px-4 sm:px-5 py-4 border-b border-gray-100">
              <div className="min-w-0">
                <h2
                  id="share-plan-title"
                  className="text-lg font-bold text-primary-900 leading-snug"
                >
                  Share plan
                </h2>
                <p className="text-xs text-text-muted mt-1">
                  Send this plan on WhatsApp or copy it.
                </p>
              </div>
              <button
                type="button"
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      copyToClipboard(planShareUrl(sharePlan), "Link copied.")
                    }
                    className="py-2.5 rounded-xl border border-gray-200 font-semibold text-sm text-slate-800 hover:bg-gray-50"
                  >
                    Copy link
            ...
            {[
              {
                ...
                answer:
                  "Each plan includes responsive design, SEO optimization, content management system, contact forms, and 24/7 support. Higher-tier plans include additional features like e-commerce functionality and advanced analytics.",
              },
              {
                icon: "clock",
                question: "How long does it take to complete a project?",
                answer:
                  "Project timelines vary based on complexity. Basic websites typically take 1-2 weeks, while more complex projects may take 3-4 weeks. We'll provide a detailed timeline during the planning phase.",
              },
              {
                icon: "chat",
                question: "Do you provide ongoing support?",
                answer:
                  "Yes! All plans include 24/7 support and regular updates. We also offer maintenance packages to keep your website secure and up-to-date with the latest features.",
              },
              {
                icon: "refresh",
                question: "Can I upgrade my plan later?",
                answer:
                  "Absolutely! You can upgrade your plan at any time. We'll work with you to add new features and functionality to your existing website without any downtime.",
              },
            ].map((faq, index) => (
              <div
                key={index}
                className={`bg-white rounded-xl p-6 border border-gray-200 shadow-sm hover:shadow-md hover:border-primary-200 transition-all duration-300 hover:-translate-y-1 ${
                  isVisible
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-8"
                }`}
